...
public class Main {
    ...
    public Main(){
        ...
        HMMparameters thisIsIt = baumWelch(transistionMatrix, emissionMatrix, initialStateVector, emissions.length-1, 
                                  emissions, 100);
    ...
    public double [][] transposeCol(double[][] colVec){
        double[][] rowVec = new double[1][colVec[0].length];
        for (int i = 0; i < colVec[0].length; i++){
            rowVec[0][i] = colVec[i][0];
        }
        return rowVec;
    }
    ...
    public double[][] alphaPass(double[][] A, double[][] B, int[] obs, double[][] pi, int n, double [][] alphaMatrix){
        ...
        //här går det fel
        double[][] notAlpha = matrixMultiplication(transposeCol(getColumn(alphaPass(A,B,obs,pi,n-1, alphaMatrix),n-1)), A);
    ...
    public double[][] betaPass(double[][] A, double[][] B, int[] obs, int n, double[][] betaMatrix) {
        if (n == obs.length-1) {
            ...
            betaMatrix = insertColInMatrix(finalCol, betaMatrix, n);
            return betaMatrix;

        }
        double[][] notBeta = elementWiseMultiplicationCol(getColumn(B, obs[n+1]), getColumn(betaPass(A, B, obs, n+1, betaMatrix),n+1));
        double[][] betaCol = matrixMultiplication(A, notBeta );
        betaMatrix = insertColInMatrix(betaCol, betaMatrix, n);
        return betaMatrix;
    }
    ...
    // row * col = row 
    public double[][] elementWiseMultiplicationRow(double[][] a, double[][] b){
    ...
    // row * col = col, Det här är så jäkla dumt 
    public double[][] elementWiseMultiplication(double[][] a, double[][] b){
        double[][] resultVector = new double[a[0].length][1];
        for (int i = 0; i<a[0].length; i++){
            resultVector[i][0] = a[0][i] * b[i][0];
        }
        return resultVector;
    }
    ...
    public HMMparameters baumWelch(double[][] startA, double[][] startB, double[][] pi, int n, 
        ...
        for (int i = 0; i < N; i++ ){
            ...
            double[][] beta = betaPass(result.getTransition(), result.getEmission(), obs, 1, betaMatrix);
